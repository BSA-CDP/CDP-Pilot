...
bool LSM6::init(deviceType device, sa0State sa0) {
    //Serial.println("Initializing IMU at fixed address 0x6B...");

    // Set device type manually if necessary
    _device = device_DS33; // or device_DSO, depending on your sensor
    address = 0x6B; // Set fixed address to detected address

    // Test communication by reading the WHO_AM_I register
    if (testReg(address, WHO_AM_I) == DS33_WHO_ID || testReg(address, WHO_AM_I) == DSO_WHO_ID) {
        //Serial.println("IMU initialized successfully at address 0x6B.");
        return true;
    } else {
        //Serial.println("Failed to verify sensor at address 0x6B.");
        return false;
    }
}
...
int16_t LSM6::testReg(uint8_t address, regAddr reg) {
    /*
    Serial.print("Testing register at address 0x");
    ...
    */
    ...
    bus->requestFrom(address, (uint8_t)1);
    if (bus->available()) {
        int16_t result = bus->read();
        //Serial.print("Register read success, value: 0x");
        //Serial.println(result, HEX);
        return result;
    } else {
        //Serial.println("Register read failed: no data available.");
        return TEST_REG_ERROR;
    }
}